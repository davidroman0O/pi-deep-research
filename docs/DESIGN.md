...
## 13. DRH review corrections (gpt-5.6-sol-wm, max thinking)

Reviewed by deep-research-heavy (agent mode, 247KB payload). Overall verdict:
**~80% correct, directionally right.** The corrections below are folded into
the implementation.

### Corrections applied

1. **Novelty gate corrected (DRH C2).** Low-novelty sources from a *new
   publisher* are now KEPT (they're corroboration), not dropped. The gate only
   skips when `low-novelty AND same-publisher AND no-new-claim`. Directly
   addresses the 4% corroboration failure.

2. **Source-family / syndication-chain detection (DRH C4/F2).** Added
   `source_family` field to Source + `detectSourceFamily()` in novel.ts.
   Wire services (Reuters, AP, Bloomberg), PR wires (PRNewswire,
   GlobeNewswire), and vendor self-publications are classified into families.
   Corroboration metrics now count *independent families*, not raw publishers —
   Reuters→blog→press-release is no longer "3 independent sources."

3. **Verify action (simplest form) wired (DRH P0 #1/#2).** After extraction,
   if a task has single-sourced high-confidence claims (≥0.6) and priority ≥5,
   a targeted corroborating search fires before the task is marked done. The
   full `choose_action` loop replaces this with a model-driven decision later.

### Corrections acknowledged but deferred

4. **Task state machine (DRH B1).** Needed to prevent action oscillation.
   Deferred until the full action loop is wired — the verify step is the
   first action beyond the fixed sequence; oscillation isn't possible yet.

5. **Event provenance / decision log (DRH B2).** Parallel workers need
   provenance tracking (who created each claim, extraction context). The
   decision log exists (`log.jsonl`) but needs richer event records.

6. **ResearchPolicyState for the planner (DRH B3).** My §11.1 defense was
   partially wrong — the snapshot needs recent actions, failed searches, query
   diversity, not just tasks/claims/coverage. Keep stateless execution but
   store policy state.

7. **Writer NOT isolated (DRH D).** Report writer is not adversarial; it
   needs full synthesis context. Only auditors/investigators need isolation.

8. **Dual retrieval paths (DRH E).** Strict hierarchy loses buried
   counterexamples. Need hierarchical drill-down + direct semantic retrieval.

9. **Claim lifecycle (DRH F4).** candidate→supported→contested→resolved→
   superseded. Code doesn't implement supersession yet.

### Build order (DRH G, confirmed)

P0: action loop → corroboration engine → evidence provenance graph.
P1: dual retrieval → claim lifecycle.
P2: specialist adversarial agents (only after loop works).
Over-engineered: persistent planner, more source-level agents, RL optimization.
