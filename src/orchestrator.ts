...
//
// Structure: runResearch() is a thin phase orchestrator — each phase is its own
// function (ensureSpec, ensureTaskGraph, runControllerLoop, buildClaimGraphPhase,
// synthesizeReport) sharing mutable state through a RunCtx. Store writes happen
// on the main thread after each parallel batch — single-writer, no races.
// Everything is resumable via disk state.
...
import { buildSnapshot, chooseAction } from "./policy.ts";
...
const LOW_NOVELTY_SATURATION = 3; // §20 — consecutive low-novelty iterations → stop
...
/**
 * Shared mutable state threaded through every phase. `meta`, `sources`, etc. are
 * object/array references, so phases mutate them in place and the next phase
 * sees the update. Built once in runResearch; passed to each phase.
 */
interface RunCtx {
	topic: string;
	deps: OrchestratorDeps;
	store: RunStore;
	meta: RunMeta; // mutated in place across phases
	config: ResearchConfig;
	handle: ModelHandle;
	search: SearchProvider;
	topicKeywords: string[];
	injectionFlags: string[]; // source urls whose content tripped injection heuristics
	progress: (line: string) => void;
	checkAbort: () => void;
}

// ── Phase 1: specification ───────────────────────────────────────────────
async function ensureSpec(ctx: RunCtx): Promise<void> {
	const { deps, store, meta, handle, topic, progress } = ctx;
	if (meta.spec) return;
	progress("Specifying research objective…");
	const raw = await llmJson<Omit<Spec, "source_policy" | "freshness">>(
		handle, SPEC_TOOL, SPEC_SYSTEM, specPrompt(topic),
...
async function ensureTaskGraph(ctx: RunCtx): Promise<Task[]> {
	const { deps, store, meta, handle, progress } = ctx;
	...
	const { tasks: raw } = await llmJson<{ tasks: Array<Omit<Task, "id" | "status" | "depth">> }>(
		handle, DECOMPOSE_TOOL, DECOMPOSE_SYSTEM, decomposePrompt(meta.spec!),
...
// ── Phase 3-5: the dynamic controller loop (§14) ──────────────────────────
//
// Schedules the ready task set, runs each task's action loop (model chooses
// search/extract/verify/summarize), then a verify safety net that corroborates
// any single-sourced high-confidence claims. Mutates tasks/sources/memos/meta
// in place. Budget and novelty-saturation counter are local — they only matter
// inside this loop.
async function runControllerLoop(
	...
	const { deps, store, meta, config, handle, search, topicKeywords, injectionFlags, progress, checkAbort } = ctx;
	...
	while (meta.stats.iterations < config.max_iterations && sources.length < config.max_sources) {
		...
		while (true) {
			...
			if (guarded.type === "search" || guarded.type === "verify") {
				...
				for (const res of ranked) {
					...
					// §20: track the best novel source ingested this iteration
					iterationMaxNovelty = Math.max(iterationMaxNovelty, nov);
					...
					const extracted = await llmJson<ExtractToolArgs>(handle, EXTRACT_TOOL, EXTRACT_SYSTEM,
					...
					const smemo = await llmJson<{ purpose: string; key_findings: string[]; limitations: string[] }>(
						handle, SOURCE_MEMO_TOOL, SOURCE_MEMO_SYSTEM,
		...
		// §20 novelty saturation: an iteration that ingested sources but none
		// cleared the novelty floor adds no new information. LOW_NOVELTY_SATURATION
		// such iterations in a row → stop. (verify-path novelty isn't counted — a
		// verify hit is corroboration by intent, not saturation.)
		if (iterationMaxNovelty > 0 && iterationMaxNovelty < NOVELTY_FLOOR) consecutiveLowNovelty++;
		else if (iterationMaxNovelty >= NOVELTY_FLOOR) consecutiveLowNovelty = 0;

		// ── verify safety net (§15 — ensures corroboration even if model didn't choose verify) ──
		await verifySafetyNet(ctx, task, sources, sourceMemos);
		...
		if (consecutiveLowNovelty >= LOW_NOVELTY_SATURATION) {
			await store.log("stopping", { reason: "novelty saturation", streak: consecutiveLowNovelty });
			break;
		}
	}
}

// ── verify safety net (§15) ───────────────────────────────────────────────
//
// After a task's action loop, find high-confidence claims still backed by only
// one source family and corroborate them with a targeted independent search.
// Mutates sources/sourceMemos/meta in place. Bounded by config.max_sources.
async function verifySafetyNet(
	...
	const { deps, store, meta, config, handle, search, progress, checkAbort } = ctx;
...
async function buildClaimGraphPhase(
	ctx: RunCtx,
	allEvidence: Evidence[],
	sources: Source[],
): Promise<{ claims: Claim[]; edges: ClaimEdge[]; contradictions: ClaimEdge[] }> {
	const { deps, store, handle, progress, checkAbort } = ctx;
	checkAbort();
	...
	const normalizationOutcomes = await runParallel(
		...
		async ([a, b]) => {
			const rel = await llmJson<{ relation: ClaimRelation | "unrelated"; reason?: string }>(
				handle, RELATION_TOOL, RELATION_SYSTEM,
	...
	const edgeOutcomes = await runParallel(
		...
		async ([a, b]) => {
			const rel = await llmJson<{ relation: ClaimRelation | "unrelated"; reason?: string }>(
				handle, RELATION_TOOL, RELATION_SYSTEM, relationPrompt(relationInput(a, b)),
				{ signal: deps.signal, temperature: 0 },
			);
	...
	return { claims, edges, contradictions };
}

// ── Phase 6b–8: syntheses → numeric → scenario → outline → sections → audits ─
//
// The linear post-loop tail. Produces the final cited markdown report, persists
// it, runs citation entailment + repair + the nine static audits, and marks the
// run completed. Returns the report text + audit report.
async function synthesizeReport(
	...
	claims: Claim[],
	edges: ClaimEdge[],
	contradictions: ClaimEdge[],
	taskMemos: TaskMemo[],
): Promise<{ report: string; audit: AuditReport }> {
	const { deps, store, meta, config, handle, progress, checkAbort, injectionFlags } = ctx;
	...
	const { syntheses } = await llmJson<{ syntheses: Array<{ dimension: string; synthesis: string; confidence: string }> }>(
		handle, TOPIC_SYNTH_TOOL, TOPIC_SYNTH_SYSTEM, topicSynthPrompt(meta.spec!, claimsByDim),
		{ signal: deps.signal, temperature: 0.3 },
	);
	...
	if (numericEvidence.length >= 3) {
		...
		const { rows } = await llmJson<{ rows: Array<{ metric: string; subject: string; value: string; normalized?: string; conditions: string; citation: number; comparable: boolean }> }>(
			handle, NUMERIC_TOOL, NUMERIC_SYSTEM, numericPrompt(meta.spec!, valueClaims),
			{ signal: deps.signal, temperature: 0.2 },
		);
	...
	if (numericEvidence.length >= 3 && /\d{4}|20\d\d|horizon|projection|future|2030|2040|2050/i.test(meta.spec!.time_horizon ?? "2035")) {
		...
		const sc = await llmJson<{ metric: string; base_value: string; scenarios: Array<{ name: string; assumption: string; projections: Array<{ year: string; value: string }> }> }>(
			handle, SCENARIO_TOOL, SCENARIO_SYSTEM,
	...
	const { sections: rawSections } = await llmJson<{ sections: Array<{ title: string; objective: string; claim_ids: string[] | string }> }>(
		handle, OUTLINE_TOOL, OUTLINE_SYSTEM,
	...
	const sectionDrafts = await runParallel(
		...
		async (section) => {
			...
			// regeneration FROM THE ORIGINAL BUNDLE with a hard length instruction;
			// the downstream completeness guard still drops anything half-written.
			...
			if (looksTruncated(draft)) {
				try {
					finalDraft = await llmText(
						handle, SECTION_SYSTEM,
						sectionPrompt(meta.spec!, section, hierarchyBundle, assumptions) +
							`\n\nHARD REQUIREMENT: complete and self-contained, under 300 words, ending with a full sentence. No tables. Cite every claim [n].`,
						{ signal: deps.signal, temperature: 0.2, maxTokens: 4000, timeoutMs: 180_000 },
					);
	...
	const methodology =
		...
		`- **Model:** ${meta.model ?? "session model"} · **Run:** ${store.runId}\n` +
	...
	const fullReport = assembled + methodology + auditNote;
	await store.saveReport(fullReport);
	...
	return { report: fullReport, audit };
}

// ── the entry point: thin phase orchestrator ──────────────────────────────
export async function runResearch(
	...
	const ctx: RunCtx = {
		...
		handle: deps.handle,
		search,
		topicKeywords: topic.split(/\s+/).filter((w) => w.length > 3).slice(0, 8),
		injectionFlags: [],
		progress,
		checkAbort,
	};
	...
	try {
		// Phase 1 + 2: specification + task graph
		await ensureSpec(ctx);
		let tasks = await ensureTaskGraph(ctx);

		// Phase 3-5: dynamic controller loop (mutates tasks/sources/memos/meta)
		...
		await runControllerLoop(ctx, tasks, sources, sourceMemos, taskMemos);

		// Phase 6: claim graph
		const allEvidence = await store.loadEvidence();
		const { claims, edges, contradictions } = await buildClaimGraphPhase(ctx, allEvidence, sources);

		// Phase 6b-8: syntheses, numeric/scenario, sectioned report, audits
		const { report, audit } = await synthesizeReport(ctx, tasks, sources, allEvidence, claims, edges, contradictions, taskMemos);
