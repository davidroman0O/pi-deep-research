...
export const EXTRACT_TOOL = {
	...
	parameters: Type.Object({
		evidence: Type.Array(
			Type.Object({
				claim: Type.String({ description: "Exactly one independently verifiable proposition. Split different metrics, outcomes, statuses, or milestones into separate evidence items." }),
				proposition_key: Type.String({ description: "Byte-stable, source-independent identity in exactly four slots: subject | predicate | value+unit | scope/date. It must map 1:1 to claim; if one key cannot fully identify every assertion, split the claim. Use lowercase ASCII, digits without thousands separators, units as stated without conversion, and none for a missing slot. Preserve every value and condition that distinguishes claims; omit attribution and source wording. Example: nuscale | foak overnight cost | 20139 usd/kw | 2022 usd. The same fact must produce the same key." }),
...
<current_as_of>${spec.freshness?.current_as_of ?? new Date().toISOString().slice(0, 10)}</current_as_of>
<time_horizon>${spec.time_horizon ?? "(unspecified)"}</time_horizon>
